...
module.exports.createRouter = () => {
    ...
    function dispatch(ctx, routes, segments) {
        ...
        if (exec) {
            ...
            if ((route = routes[segment])) {
                ...
                if (handler) {
                    return instantiate(handler, segments, ctx.url.substring(lastIndex));
                }
            }
    ...
    function instantiate(handler, segments, url) {
        ...
        const params = {};
        if (handler.params) {
            for (let i = handler.params.length - 1; i >= 0; i--) {
                const name = handler.params[i];
                params[name] = decodeURIComponent(segments[i]);
            }
        }
        ...
        return {
            ...
            fragment,
            exec() {
                return handler.apply([{...params, ...query}, ...arguments]);
            }
        }
    }
