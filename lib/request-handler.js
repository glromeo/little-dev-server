...
module.exports.createRouter = () => {
    ...
    function getRoute(prototype, segments) {
        const params = prototype.params;
        if (params) {
            const route = Object.create(prototype);
            route.params = {};
            for (let i = params.length - 1; i >= 0; i--) {
                route.params[params[i]] = segments[i];
            }
            return route;
        } else {
            return prototype;
    ...
    function dispatch(ctx, current, segments) {
        ...
        if (exec) {
            ...
            if ((route = current[segment])) {
                ...
                if (next) {
                    return getRoute(next, segments);
                }
            }
            if ((route = current[MATCH_ONE])) {
            ...
            if ((route = current[MATCH_MANY])) {
                let next = route[ctx.method];
                if (next) {
                    const route = getRoute(next, segments);
                    route.rest = ctx.url.substring(lastIndex);
                    return route;
                }
            }
        }
    ...
    const router = {
        ...
        use(method, path, handler) {
            const re = /\/+([^/?#]*)/g;
            let ctx = this.routes;
            let s = 0, route = {params: [], handler}, exec, segment;
            while (exec = re.exec(path)) {
                segment = exec[1];
                if (segment.startsWith(":")) {
                    ctx = ctx[MATCH_ONE] || (ctx[MATCH_ONE] = {});
                    route.params.push(segment.substring(1));
                } else if (segment === "**") {
        ...
        route(method, url, req, res) {
            ...
            if (route) {
                route.handler(route.params, route.rest);
            }
        }
    }
