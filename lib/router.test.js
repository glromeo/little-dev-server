describe("router", function () {
    ...
    describe("unit test", function () {
        ...
        it("can register routes for various http methods according to specificity", () => {
            ...
            // note that "4" and "5" are more specific than "1" and "2"
        ...
        it("can handle path params", async function () {
            ...
            req = request("/abc/def");
            expect(await router.route(req)).toMatchObject({
                url: "",
                handler: mock1,
                variables: {name: "def"}
            });
            ...
            req = request("/abc/def/ijk", {method: "PUT"});
            expect(await router.route(req)).toMatchObject({
                url: "",
                handler: mock2,
                variables: {alias: "ijk"}
            ...
            expect(await router.route(req)).toMatchObject({
                variables: {name: "xxx", address: "yyy"}
            });
        });
        ...
        it("can handle get parameters and ** to match the rest of the url", async function () {
            ...
            req = request("/abc/def");
            expect(() => router.route(req)).toThrowError("no route found for: GET /abc/def");
