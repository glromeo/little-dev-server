...
const {quickParseURL, isBare, nodeModuleBareUrl, parsePathname, toPosix} = require("./quick-parse-url.js");
...
module.exports.useWebModules = memoize(config => {
    ...
    const standalone = new Set(config.web_modules.standalone);
    ...
    const pluginTerser = config.web_modules.terser && createPluginTerser(config.web_modules.terser);
