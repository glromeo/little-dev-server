...
const {quickParseURL} = require("../utility/quick-parse-url.js");
...
module.exports.createRouter = function (config) {
    ...
    function mount(path, roots) {
        ...
        for (let root of Array.isArray(roots) ? roots : [roots]) if (!route.has(root)) try {
            fs.mkdirSync(resolve(config.rootDir, root), {recursive: true});
            const ls = fs.readdirSync(root).filter(dir => dir.charCodeAt(0) !== FullStop);
