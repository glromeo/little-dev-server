...
const {FullStop} = require("../util/char-codes.js");
const {quickParseURL} = require("../util/quick-parse-url.js");
...
module.exports.createRouter = function (config) {
    ...
    function mount(path, roots) {
        ...
        for (let root of Array.isArray(roots) ? roots : [roots]) if (!route.has(root)) try {
            ...
            const ls = readdirSync(root).filter(dir => dir.charCodeAt(0) !== FullStop);
