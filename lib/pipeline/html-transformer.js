...
const {isBare, posixDirname} = require("../utility/quick-parse-url.js");
...
module.exports.useHtmlTransformer = memoize(config => {
    ...
    const transformHtmlAsync = async (filename, content) => new Promise(async (resolve, reject) => {
        ...
        const base = posixDirname(filename);
            ...
            onopentag(name, attribs) {
                ...
                if (name === 'script') {
                    ...
                    if (type === "module") {
                        ...
                        if (src && src.value) {
                            html.push(
                                resolveImport(base, src).then(relativeUrl => {
    ...
    const htmlTransformer = blockingTransformer(async (filename, content) => {
