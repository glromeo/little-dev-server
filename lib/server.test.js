...
describe("server", function () {
    ...
    it("can start/stop an http server", async () => {
        ...
        const {server, address} = await startServer(configure({logLevel: "debug", http2: false, onexit: onexit}));
        const response = await new Promise(resolve => request("address").get("/health", resolve));
        const health = JSON.parse(await contentText(response));
