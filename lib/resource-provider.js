...
module.exports.useResourceProvider = memoize(function (config, watcher) {
    ...
    async function provideResource(url) {
        ...
        const output = !module && await transformResource(headers["content-type"], filename, content, query);
        if (output) {
        ...
        headers["cache-control"] = module ? "public, max-age=86400, immutable" : "no-cache";
