...
const {posixDirname} = require("./quick-parse-url.js");
...
module.exports.useWebModules = memoize(config => {
    ...
    async function resolveImport(base, url) {
        ...
        if (ext !== ".js" && ext !== ".mjs") {
            ...
            } else if (pathname.charCodeAt(0) === Slash) {
                const resolved = await resolveAsync(path.join(config.rootDir, url.substring(1)), customResolveOptions);
                pathname = "/" + toPosix(path.relative(config.rootDir, resolved));
            } else {
                const resolved = await resolveAsync(path.join(base, url), customResolveOptions);
                pathname = toPosix(path.relative(base, resolved));
                if (pathname.charCodeAt(0) !== FullStop) {
                    pathname = "./" + pathname;
                }
            }
        }
                    ...
                    resolveId(source, from) {
                        if (isValidPathname(source) && isValidPathname(from)) {
                            if (isBare(source)) {
                                ...
                                const bare = nodeModuleBareUrl(path.resolve(from, source));
