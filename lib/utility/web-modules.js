...
/**
 * module is "." if the path is relative, undefined if it's absolute
 *
 * @param url
 * @returns {{ext: string, filename: string, module: string}|{ext: undefined, filename: undefined, module: *}|undefined}
 */
const parseWebModuleURL = function (url) {
    ...
    if (sep === -1) {
        ...
        if (url.charCodeAt(sep - 1) === Colon && url.charCodeAt(sep + 1) === Slash) return {
            url
        }
        ...
        if (cc === Slash) {
            ...
            if (dot > 0) {
                filename = url.substring(start, dot);
                ext = url.substring(dot);
            } else {
            ...
            module = url.substring(0, sep);

            if (dot > 0) {
                filename = url.substring(sep + 1, dot);
                ext = url.substring(dot);
            } else {
                filename = url.substring(sep + 1);
                ext = undefined;
            }
        }
    }
